          ...
          case DbUser(usernameCheck.user_id, usernameCheck.first_name, usernameCheck.last_name, usernameCheck.date_of_birth, usernameCheck.username, usernameCheck.email, usernameCheck.password) => if (
            ...
            request.session.get("clickwithoutlogin").map { log =>
              ...
              if (log.contentEquals("discussion")) {
                Redirect(routes.CommentController.index()).addingToSession("userid" -> usernameCheck.user_id.toString).discardingCookies(DiscardingCookie("usercheck")).discardingCookies(DiscardingCookie("passcheck")).removingFromSession("clickwithoutlogin")
                ...
                Redirect(routes.BookingController.toBookings()).addingToSession("userid" -> usernameCheck.user_id.toString).discardingCookies(DiscardingCookie("usercheck")).discardingCookies(DiscardingCookie("passcheck")).removingFromSession("clickwithoutlogin")
              ...
              Redirect(routes.HomeController.index).addingToSession("userid" -> usernameCheck.user_id.toString).discardingCookies(DiscardingCookie("usercheck")).discardingCookies(DiscardingCookie("passcheck"))
